...
- **Open catalog.** Run `node .agents/skills/verify-dvns-integrated-sources/scripts/verify.mjs drive integrated-data-catalog`. The H1 is `Tutti i dataset integrati` and 79 unique detail links are present.
...
- **Proof.** Retain `catalog.png`, `consulenze-legali.png`, `api-response.json` and `state.json` in the feature evidence directory.
