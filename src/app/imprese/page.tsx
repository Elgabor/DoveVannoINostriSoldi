...
export const metadata: Metadata = {
  ...
  description:
    "Mappa e dati aggregati del tessuto produttivo italiano: imprese attive, addetti, localizzazioni e fasce di valore della produzione.",
};
...
function atlasHref(view: { metric: string; period: string; region: string; sector: string; band: string }, region?: string) {
  ...
  if (view.sector !== COMPANY_ATLAS_ALL) params.set("sector", view.sector);
  if (view.band !== COMPANY_ATLAS_ALL) params.set("band", view.band);
  if (region && region !== COMPANY_ATLAS_ALL) params.set("region", region);
...
export default async function ImpresePage({
  ...
  const filterOptions = {
    metrics: companyAtlasMetricOptions().map((item) => ({ id: item.id, label: item.label })),
