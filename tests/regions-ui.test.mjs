...
test("Regions page uses only the verified Istat regional account", () => {
  ...
  assert.match(page, /soldi impegnati, non i pagamenti/);
  assert.match(page, /non li\s*\n?\s*mescoliamo con Comuni, sanità o altri conti/);
...
test("Regions page exposes a URL filter, exact table and honest comparison limits", () => {
  ...
  assert.match(page, /Valori esatti dei soldi impegnati nel 2024/);
  assert.match(page, /siopeTitleCopy/);
...
test("Regional treemap is additive and exact tables remain internally scrollable", () => {
  ...
  assert.match(treemap, /soldi impegnati nel 2024/);
