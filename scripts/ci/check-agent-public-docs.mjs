...
export function checkAgentPublicDocs(snapshot) {
  ...
  for (const card of cards) {
    ...
    if (card?.httpEndpoint) {
      if (!card.httpExampleQuery || Object.keys(card.httpExampleQuery).length === 0) {
        add(id, "http-example", "API HTTP dichiarata senza esempio italiano");
      }
    } else if (card?.httpExampleQuery && Object.keys(card.httpExampleQuery).length > 0) {
      add(id, "http-example", "esempio HTTP dichiarato senza API HTTP supportata");
    }
    ...
    if (!parsed.success) {
      add(
        ...
        "example",
        published
          ? "esempio pubblicato non accettato da datasetQuerySchema"
          : "esempio del descrittore non accettato da datasetQuerySchema; il titolo di eccezione generato non costituisce un contratto deliberato",
      );
    }
  ...
  const cardMarkdowns = snapshot.cardMarkdowns;
  if (!Array.isArray(cardMarkdowns)) {
    add(null, "card-markdowns", "cardMarkdowns mancante: il Markdown finale delle schede non è coperto");
  } else {
    const markdownById = new Map(cardMarkdowns.map((cm) => [cm?.id, cm?.markdown]));
    for (const id of activeIds) {
      if (!markdownById.has(id)) {
        add(id, "markdown-coverage", "Markdown finale mancante per dataset attivo");
      }
    }
    for (const { id, markdown } of cardMarkdowns) {
      const card = cards.find((c) => c?.id === id);
      if (!markdown) {
        add(id, "markdown", "Markdown finale mancante");
        continue;
      }
      if (!markdown.includes("MCP:")) {
        add(id, "markdown-mcp", "Markdown finale non dichiara l'accesso MCP");
      }
      if (card?.httpEndpoint) {
        if (!markdown.includes("API HTTP esistente")) {
          add(id, "markdown-http", "Markdown finale con API HTTP non dichiara l'accesso HTTP");
        }
        if (!markdown.includes("Esempio HTTP")) {
          add(id, "markdown-http-example", "Markdown finale con API HTTP non dichiara l'esempio HTTP");
        }
      } else {
        if (markdown.includes("API HTTP esistente") || markdown.includes("Esempio HTTP")) {
          add(id, "markdown-http-unexpected", "Markdown finale promette accesso HTTP per un dataset senza API HTTP");
        }
      }
    }
  }
...
export async function buildAgentPublicDocsSnapshot() {
  ...
  const cards = docs.listAgentPublicDocs();
  return {
    ...
    requiredIndexLinks: REQUIRED_INDEX_LINKS,
    cards,
    cardMarkdowns: cards.map((card) => ({ id: card.id, markdown: docs.renderAgentPublicDocMarkdown(card) })),
    datasetPathPrefix,
